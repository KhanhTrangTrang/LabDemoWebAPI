...
namespace LabDemoWebASPMVC.Controllers
{
    ...
    public class UsersController : Controller
    {
                                               ...
                                               int? pageNumber)
        {
            try
            {
                HttpResponseMessage response = await client.GetAsync("http://10.1.11.102:8098/api/employees");
        ...
        public async Task<IActionResult> EditUser(Employee user)
        {
            ...
            HttpResponseMessage response = await client.PutAsync("http://10.1.11.102:8098/api/employees", content);
            // Case change email
        ...
        public async Task<IActionResult> AddUserPostAsync(Employee user)
        {
            ...
            HttpResponseMessage response = await client.PostAsync("http://10.1.11.102:8098/api/employees", content);
            // Case change email
            if (response.StatusCode == System.Net.HttpStatusCode.BadRequest)
            {
        ...
        {
            JsonContent content = JsonContent.Create(user);
            HttpResponseMessage response = await client.PutAsync(string.Format("http://10.1.11.102:8098/api/employees/{0}", user.Id), content);
        ...
        {
            JsonContent content = JsonContent.Create(user);
            HttpResponseMessage response = await client.DeleteAsync(string.Format("http://10.1.11.102:8098/api/employees/{0}", user.Id));
        ...
        public async Task<IActionResult> DetailUserAsync(int id)
        {
            ...
            HttpResponseMessage response = await client.GetAsync(string.Format("http://10.1.11.102:8098/api/employees/{0}", id));
            response.EnsureSuccessStatusCode();
            string responseBody = await response.Content.ReadAsStringAsync();
